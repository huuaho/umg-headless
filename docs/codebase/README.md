...
Key flows:
- **Content (UMG):** the [united-media-ingestor](plugin/united-media-ingestor/README.md) plugin pulls articles from the EM/IS backends + Diplomatic Watch into a local `um_article` CPT via cron, and serves them at `GET /wp-json/um/v1/articles` (items carry a `video_url` field since plugin 0.10.0). [@umg/api](packages/api/README.md) fetches them in `custom` mode; articles link externally (blank `slug` convention → [ArticleLink](packages/ui/ArticleLink.tsx.md) renders `<a target="_blank">`). IS video interviews map to a dedicated `video-interviews` UM category that feeds the UMG homepage's Video Interviews section.
...
- **Photo competition (UMG), individual flow:** email-OTP auth → JWT → autosaved draft → photo uploads → submit → Stripe payment-link, polled via `GET /me`. Frontend in [apps/umg/lib/auth + photo-submission](apps/umg/README.md), backend in [umg-photo-contest](plugin/umg-photo-contest/README.md) (`/wp-json/umg/v1/`).
- **Photo competition (UMG), school/bulk-registration flow:** same auth, but one account manages many independent applications and pays **once for the whole batch** via a dynamically created Stripe Checkout Session (not a static payment link) — the webhook credits every covered application from a single settlement event. Frontend in `apps/umg/app/school-registration` + `apps/umg/lib/school`, backend in [umg-photo-contest](plugin/umg-photo-contest/README.md) (`/wp-json/umg/v1/school/*`). Feature-complete and live-verified with a real payment (2026-07-03).
