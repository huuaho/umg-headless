...
**Purpose:** Global stylesheet — Tailwind 4 setup, International Spectrum theme variables, and the marquee banner animation.
...
## Key exports
- (CSS) `--banner-border-color: #feb70c` — International Spectrum yellow accent used by the shared Header/Footer banner borders.
- (CSS) `--footer-bg: #e8e6f2` — light purple Footer background.
...
- **Difference vs echo-media:** only the variable values — EM uses `--banner-border-color: #0281b3` (blue) and `--footer-bg: #d9ecf3` (light blue). All other rules are byte-identical.
