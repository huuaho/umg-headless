...
2. **Payment view** (submitted + unpaid): shows entry fee, a Stripe payment-link button (`stripePaymentLink` with `?prefilled_email=` so the WP webhook can match the payer), a manual "check status" button, and polls `refreshUser()` every 15 s until `payment_status === "paid"`.
