...
**Purpose:** The school's application manager — list, add, delete, reopen-for-edit, and pay for the whole batch in one checkout.

## Responsibilities
Lists every application the signed-in school owns (name, division, status, payment status), with edit/view and delete actions per row, a **"Reopen to edit"** action on rows that are submitted-but-unpaid (calls `unsubmitApplication`, then navigates to the application form; a footnote explains applications stay editable until paid), an "Add another application" button, and a single **"Pay $Z total (N students)"** button — not one payment per student — shown whenever at least one application is submitted-and-unpaid. Polls the list every 15s (same interval as the individual flow's own polling) while any submitted application remains unpaid, so a completed Stripe payment shows up without a manual refresh. Reads a `?checkout=success`/`?checkout=cancelled` query param (from [lib/school/api.ts](../../../lib/school/api.ts.md)'s checkout `success_url`/`cancel_url`) to show a one-time banner on return from Stripe.

## Key exports
- `default ApplicationsCart() -> JSX`

## Dependencies
- Internal: [lib/auth/AuthContext](../../../lib/auth/AuthContext.tsx.md) (`useAuth`), [lib/school/api](../../../lib/school/api.ts.md) (`listApplications`, `createApplication`, `deleteApplication`, `unsubmitApplication`, `createCheckoutSession`), [lib/school/types](../../../lib/school/types.ts.md) (`ApplicationSummary`)
- External: `next/navigation` (`useRouter`, `useSearchParams`), React (`useState`, `useEffect`, `useCallback`, `useRef`)

## Used by
Rendered by [../page.tsx](../page.tsx.md) once the school is signed in (that page currently `notFound()`s while the competition is postponed, so this component is unreachable in production right now).
...
- **Editable until paid** ("added school app edits before payments" commit): `handleReopen(id)` reuses `pendingId` as its busy flag (so Reopen/Delete on the same row disable together), clears the error, calls `POST /school/application/{id}/unsubmit`, and `router.push`es to `/school-registration/application?id=<id>`. Paid applications never show the button — the plugin also refuses to unsubmit them server-side. Mirrors the individual flow's "Back to edit my submission" in [SubmissionForm](../../photo-submission/components/SubmissionForm.tsx.md).
- **Load-failure vs. genuinely-empty are distinguished** (`loadFailed` state, landed in the same commit): a `loadFailed` state renders "Couldn't load your applications" + a "Try again" button, separate from the real "No applications yet" empty state — before this fix, a failed initial load looked identical to a school having zero applications.
