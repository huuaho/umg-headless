...
- **Load-failure handling** (`loadFailed` state; landed with the "added school app edits before payments" commit): a failed `getApplication` call previously fell through to a blank editable form with only a small error line near the submit button. Now shows a dedicated "Could not load this application" screen (Try again / Back to applications) before the form renders at all. The load logic is a `useCallback`d `loadApplication` function (deps `[token, applicationId]`, with an eslint exhaustive-deps suppression because `competition.divisions` is a stable module constant) so "Try again" can re-trigger it without duplicating the effect body. It also stores `paymentStatus` from the response.
...
- Unreachable in production right now: [../application/page.tsx](../application/page.tsx.md) `notFound()`s while the competition is postponed.
