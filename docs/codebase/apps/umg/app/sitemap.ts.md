...
## Responsibilities
A Next.js App Router metadata route (`force-static`, compatible with `output: 'export'`). Emits a `MetadataRoute.Sitemap` array: a hardcoded list of the main static routes (`/`, `/about-us`, `/contact`, `/search`, and `/how-to-enter` — re-added 2026-08-23 now that it serves the competition on-hold announcement; `/judges-panel` and `/photo-submission` stay commented out while the competition is postponed) with per-route `changeFrequency`/`priority`, plus one entry per category — mapped from `pageCategories` in `lib/categories` (the 8 nav categories + Video Interviews) so adding a category updates the sitemap on rebuild. All URLs use the canonical `https://unitedmediadc.com` base with a trailing slash (matching `trailingSlash: true`).

## Key exports
- `default sitemap() -> MetadataRoute.Sitemap` — the route list (14 URLs as of this commit — 5 static + 9 categories).
