...
- `unsubmitApplication(token, id) -> void` — `POST /school/application/{id}/unsubmit`; reverts a submitted-but-unpaid application to draft (the plugin rejects it once paid).
...
- `unsubmitApplication` is the school-flow twin of `unsubmitEntry` in [../auth/api.ts](../auth/api.ts.md); both back the "editable until paid" behaviour shipped in the `3e9c801`/`fa4b6ac` commits.
