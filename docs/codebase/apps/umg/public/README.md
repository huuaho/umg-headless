...
| images/banner/ | folder | Media-company marquee logos — color + B&W variant per company (em, is, dw), referenced by [lib/mediaCompanies.ts](../lib/mediaCompanies.ts.md). Local copies replaced WP-hosted uploads (see `docs/components/banner-assets.md`). |
