...
- Renders [SectionSkeleton](SectionSkeleton.tsx.md) on initial load, [SectionError](SectionError.tsx.md) with retry on error/empty (unless `hideWhenEmpty`: an error-free empty category then renders `null` instead of the "No articles found" card — for sections whose backend content may ship after the frontend), `null` if dedup consumed everything, otherwise transforms via `toSectionData` / `toSectionType3Data` / `toSectionType4Data` ([transformers](../../api/transformers.ts.md)) and renders [SectionType1](SectionType1.tsx.md)–[4](SectionType4.tsx.md).
...
## Key exports
- `CategorySectionWrapper({ slug, category, sectionType, categoryColor?, categoryTextColor?, categoryUnderlineColor?, categoryIcon?, titleClassName?, priority?, latest?, hideWhenEmpty? })` (default).
...
## Used by
- All three apps' homepage `app/page.tsx` — one wrapper per category section, configured from each app's category list. [International Spectrum](../../../apps/international-spectrum/app/page.tsx.md) and [UMG](../../../apps/umg/app/page.tsx.md) additionally render one `latest` wrapper at the top of the page; UMG also renders a Video Interviews wrapper with `hideWhenEmpty` and `priority={0}`.
