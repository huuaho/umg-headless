...
| Item | Type | Summary |
|------|------|---------|
| [CategorySectionWrapper.tsx](CategorySectionWrapper.tsx.md) | file | Smart orchestrator: `useArticles` fetch → seen-articles dedup → transformer → SectionType1–4 / skeleton / error. `latest` prop = recency mode (newest posts site-wide, unlinked label); `hideWhenEmpty` renders nothing for an empty category. |
...
- A `CategorySectionWrapper latest` renders a "Latest" (newest-across-all-categories) section; IS and UMG each use one at the top of their homepage, without `priority` so it stays out of dedup. All four SectionType components accept `noLabelLink` for this case.
- UMG's Video Interviews section uses `hideWhenEmpty` so it disappears (rather than showing the empty-state card) until the backend category has posts.
