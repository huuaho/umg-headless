...
## Responsibilities
- Renders the section shell (`id={slug}`, `scroll-mt-24` for hash navigation) with a [CategoryLabel](CategoryLabel.tsx.md) (linked to `/category/<slug>` unless `noLabelLink` is set).
...
## Key exports
- `SectionType1(props)` (default) — `SectionData` fields (`featured`, `secondary`) plus `slug`, `category`, `noLabelLink?` (render the label unlinked — used by the recency "Latest" section), category-label styling props, and `titleClassName`.
