...
## Responsibilities
- Same building blocks as [SectionType1](SectionType1.tsx.md): [CategoryLabel](CategoryLabel.tsx.md) (unlinked when `noLabelLink` is set), featured title/snippet/meta + [FeaturedMedia](components/FeaturedMedia.tsx.md), 4 secondary cards, all linked via [ArticleLink](../ArticleLink.tsx.md).
...
## Key exports
- `SectionType2(props)` (default) — `SectionData` fields plus `slug`, `category`, `noLabelLink?`, category-label styling props, `titleClassName`.
