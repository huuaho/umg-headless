...
- `umgpc_get_draft` — returns the full draft: `status`, `division`, name/DOB/address/school/grade/job/biography, `recommender` (optional free-text field added 2026-07 with the "deadline → Oct 31" content update; identity-gated in the judge panel), `photos`, `student_proof`, five consent booleans (`consent_originality/subjects/rights/rules/social_media`), `social_links`, `submitted_at`. 404 if no draft.
...
## Notes
- Post meta written: `umgpc_user_id`, `umgpc_status`, all text/consent fields (incl. `umgpc_recommender`), `umgpc_photo_{1-3}_{id,title,description}`, `umgpc_student_proof_id`, `umgpc_social_links`. Note that `post_author` is **not** set on the created post (no `wp_set_current_user()` under JWT auth) — ownership is `umgpc_user_id` meta only; see [entry-state.php](entry-state.php.md) Notes for why that matters to `umgpc_entry_is_paid()`.
...
- Upload responses use key `id` (not `media_id`); the old plugin doc claims `media_id`. Modification of a submitted entry returns HTTP 400 (not 403 as the old doc says). The `already_submitted` lock is no longer permanent: `POST /unsubmit` ([submission.php](submission.php.md) → [entry-state.php](entry-state.php.md)) reverts the entry to `draft` until it is paid, after which it is final.
