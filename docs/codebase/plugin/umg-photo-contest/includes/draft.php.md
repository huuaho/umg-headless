...
## Key exports
- Helpers: `umgpc_find_draft_id($user_id) -> int` (WP_Query on `umgpc_user_id` meta), `umgpc_create_draft($user_id) -> int|WP_Error`, `umgpc_build_photos_array($post_id) -> array` (slots 1–3 → `{media_id, url, title, description}`), `umgpc_build_student_proof($post_id) -> array|null` (`{media_id, url, filename}`).
