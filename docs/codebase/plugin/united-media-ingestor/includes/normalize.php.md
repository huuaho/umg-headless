...
## Responsibilities
Transforms pieces of a `wp/v2/posts?_embed=1` payload into clean values: category names from `_embedded['wp:term']`, decoded plain-text titles/excerpts, a best-effort GMT date, gallery attachment IDs parsed out of shortcodes, full-size image URLs scraped from content HTML, and a YouTube thumbnail URL derived from a video link.
...
- `um_youtube_thumbnail_url($video_url) -> string` — regex-extracts the 11-char video ID from watch/embed/shorts/youtu.be URLs and returns the `img.youtube.com/vi/<id>/hqdefault.jpg` URL (`''` for non-YouTube links). Uses `hqdefault` deliberately — it exists for every video, unlike `maxresdefault`.
...
## Used by
[storage.php](storage.php.md) (`um_upsert_article` calls the category/gallery/image extractors, and `um_youtube_thumbnail_url` as the featured-image fallback for video posts) and [admin-endpoints.php](admin-endpoints.php.md) (image-refresh AJAX handler re-runs the gallery/image extraction). Category names feed [mapping.php](mapping.php.md)'s `um_resolve_categories`.
