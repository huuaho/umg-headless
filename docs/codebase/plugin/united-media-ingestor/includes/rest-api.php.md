...
## Used by
The UMG frontend via the shared API client ([packages/api/client.ts](../../../packages/api/client.ts.md), types in packages/api/types.ts) — homepage category sections (including the Latest and Video Interviews sections added with plugin 0.10.0, the latter filtering on `category=video-interviews` and using `video_url`), search, and article cards on www.unitedmediadc.com.
